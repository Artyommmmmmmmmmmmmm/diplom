import { 
    idComplaintRetrieveAction, idTMRetrieveAction, idMachineRetrieveAction,
...
export const fetchMachineIdRetrieve = (id) => {
    return function(dispatch) {
        $api.get('main/machines/' + (id + 1))
            .then(response => dispatch(idMachineRetrieveAction(response.data)))
            .catch(err => console.log(err))
    }
}
export const fetchTMIdRetrieve = (id) => {
    return function(dispatch) {
        $api.get('main/tms/' + (id))
            .then(response => dispatch(idTMRetrieveAction(response.data)))
            .catch(err => console.log(err))
    }
}
